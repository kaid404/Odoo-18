...
class ProfessionalDetailsManagement(models.Model):
    ...
    emp_board_uni = fields.Char(string='Board / University')
    emp_cgpa = fields.Float(string='CGPA')
    emp_obt_marks = fields.Integer(string='Obtained Marks')
    emp_country_name = fields.Many2one('res.country', string='Country')
    emp_from_date = fields.Date(string='From')
    emp_to_date = fields.Date(string='To')
...
class BodyVerificationDetailsManagement(models.Model):
    ...
    emp_body_verification_status = fields.Selection([('verified', 'Verified'), ('not_verified', 'Not Verified'), ('in_progress', 'InProgress')], string='Verification Status')
    emp_body_verification_date = fields.Date(string='Verification Date')
    emp_body_verification_remarks = fields.Date(string='Remarks')
