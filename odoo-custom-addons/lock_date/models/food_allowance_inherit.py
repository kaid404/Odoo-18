...
class HrPayslip(models.Model):
            ...
            for val in vals_list:
                ...
                if date:
                    ...
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Allowances before {lock_date.lock_date} cannot be created.")
    ...
    def write(self, vals):
        ...
        if lock_date:
            for rec in self:
                new_date = vals.get('date') or rec.date
                if new_date:
    ...
    #         for rec in self:
    #             if rec.date:
    #                 if rec.date < lock_date.lock_date:
    #                     raise UserError(f"Allowances before {lock_date.lock_date} cannot be modified.")
    #     return super().write(vals)
        ...
        if lock_date:
            for rec in self:
                if rec.date:
                    date_in = rec.date
                    if date_in < lock_date.lock_date:
                        raise UserError(f"Allowances before {lock_date.lock_date} cannot be deleted.")
        return super().unlink()